...
This is the most important move in the chapter, please make it a habit. Chapter 9 explained that when it isn't sure, it will **string together a plausible fake answer along the inertia of language**. When you feed material for it to summarize, the thing you fear most is it "summarizing" in content **that isn't in the material at all.**
...
Most material work falls into these three kinds. Each has its own phrasing, so memorize the routine and you can use it on the spot.
...
- "How long is long" is measured in tokens, but that's a **rough rule of thumb**, varies by model, and the window keeps changing, for precision use the official counting tool (check the official docs).
...
4. **[Advanced · Misconception]** "The internet says 1000 characters is about 1000-plus tokens, so if I memorize that ratio I can precisely calculate whether the window will overflow." What's off?
   ...
   > **Answer: B.** Chapter 3 stresses repeatedly: this is a rough rule of thumb, not a law, models can differ by twenty or thirty percent, and window sizes keep changing (check the official docs). Whoever treats it as a fixed ratio will miscalculate at the edge. C is also wrong, "which uses more tokens" is no longer a rule on new tokenizers; D is the most common misconception (a token is not a character).
...
6. **[Basic · Hands-on]** Find a long article you've read and know well. First have it "summarize into 5 key points" with **no** restriction; then redo it with "summarize from this article only, don't supplement what isn't in the material, and mark which paragraph of the original each point comes from." Compare the two, especially watching for content that "isn't actually in the original" slipping into the first summary.
   > **What you should notice:** Without the leash, it sometimes mixes in "reasonable extensions" not in the original or content it outright made up, that's hallucination, and it confirms "looks complete does not equal faithful to the original"; with "answer from the material only + mark sources" added, the drift drops noticeably and you can spot-check more easily. Compare once by hand and you'll never again blindly trust a summary it rattles off, which is exactly the core idea of Chapter 20's RAG: feed the right material in front of it.
