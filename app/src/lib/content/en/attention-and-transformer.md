...
When we read a long sentence, our eyes don't sweep evenly over every character. Reading a given word, we automatically look more at the few words related to it, "underline" what matters, and just glance past the rest.
...
> **How you'll actually run into this:** You'll almost never bump into the word "Transformer" directly while using AI; it hides at the very bottom. But you'll see it again and again in tech news and product write-ups. Next time you see it, just have it clear in your mind: ah, that's the "engine model" these models share, not some new product or piece of magic tech. What changes each company has made to this architecture, and exactly what theirs looks like, are technical details that shift over time, so check the official docs / technical reports.
...
> **Key point:** "It can grab what matters" and "it understood" are two different things. The first is true (attention really does work); the second is an illusion (underneath there's only mathematical weighting, no understanding). This distinction runs through the whole book. It's also one of the roots of why it can "state errors with a straight face" (the hallucination of Chapter 9): a system that doesn't truly understand and is only reproducing statistical patterns, when tripped up by a rare case, **can't tell** anything is wrong.
...
| The Transformer is a specific AI product | It's an **underlying architecture** (an engine model); Claude, GPT, Gemini, and others are all built on it |
...
- It also **looks with several sets of eyes at once** (reference, action, cause-and-effect each handled separately), so its understanding is fairly three-dimensional.
- The **Transformer** is the underlying architecture of nearly every mainstream large model today, like an "engine model," with attention at its core. Knowing such a thing exists is enough; its internal structure isn't worth digging into.
...
2. **[Basic · Concept]** Which statement about the Transformer is correct?
   ...
   > **Answer: B.** The Transformer is like an "engine model"; Claude, GPT, Gemini, and others are all built on it, with attention as the core mechanism. A treats the architecture as a product (a classic mix-up); D is the daunted misconception most worth clearing up, since inside it's all math and ordinary users need not dig in at all.
