...
- **Get an "API key."** Using an API usually means first applying for a key on the relevant platform, used to prove "it's you placing the order," and billing **by token** accordingly (Chapters 3 and 38). For the exact application process, **refer to the official documentation.**
...
Below we break "connecting to an API" into a few steps, all covering only the approach and the data flow. For the specific language, how to write it, and which interface to call, **refer to the official documentation.**
...
| What to do | Why |
| --- | --- |
| **Put the key in an "environment variable," don't write it into the code** | Hard-coded into the source, the key travels everywhere the code goes and can leak at any moment |
...
| **Assuming the model always sticks to the format you asked for** | It makes mistakes and goes off track (Chapter 9). The program has to check and have a fallback for the returned result; don't run it bare |
| **Sending users' private data to the API as is** | Remember that line from Chapter 37: **sending it in is the same as handing it over.** Sensitive information has to be de-identified first; whether it's used for training depends on the terms and settings (Chapter 37; refer to the official documentation) |
| **Copying some old code snippet from the internet wholesale** | Each provider's API syntax differs and updates often. **Refer to the official documentation**; don't put faith in outdated code snippets |
...
- **Take in more structured output**: have the model return a tidier structure (so the program can use it directly); for exactly how to ask, **refer to the official documentation.**
...
- The smallest runnable data flow is one line: **your program, carrying the key, to the model, the answer passed back, the program uses it.** This chapter covers only this approach, pinning down no language's SDK code (**refer to the official documentation**).
...
- What the model returns can still be wrong and can still ignore the format (Chapter 9), so the program has to check and have a fallback; data you send in is the same as handing it over, so de-identify sensitive information first (Chapter 37).
...
5. **[Basic · Scenario]** Your program uses the API to have the model "answer only 'positive' or 'negative,'" but once in a while it replies with a whole sentence of explanation. What should you do?
   ...
   - B. Check and have a fallback for the returned result in the program; don't assume the model always sticks to the format
   - C. Swap in a new key
   - D. Conclude the API is broken
   > **Answer: B.** What the model returns can still be wrong and can still ignore the format (hallucination happens just the same, Chapter 9). A robust program assumes "it may not obey," checking and having a fallback for the return value, instead of running bare and assuming it's always right.

6. **[Basic · Hands-on / Observation]** You don't have to actually write code. Draw the "connecting to an API" data flow on paper: from "your program sends the question out (carrying the key)," to "the model passes the answer back," to "your program uses the answer." Then mark three points on the diagram about money/safety: where the key goes, where billing happens, where the cap is set. (For the specific syntax, **refer to the official documentation.**)
