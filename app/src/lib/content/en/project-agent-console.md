...
> [This chapter is about the general approach to "handling an agent that runs on its own," not tied to any one platform. Which product, which interface you build it in, how you configure the tools, all differ from provider to provider and update fast, so always **check the official docs**.]
...
- **Have an environment you can fall back on ready.** An agent really does act, so **the things it can touch should ideally all be recoverable**. If local files are involved, try it in a separate folder managed with Git (echoing Chapters 24 and 27); if you use a platform's built-in agent, look at the isolation or sandbox it provides, and **check the official docs.**
- **The single most important thing: make a "must-stop list."** Before letting it run, write down "which steps can only be done with my nod." This list is the soul of this chapter, and Section 2 below walks you through making it step by step.
...
The process below doesn't depend on any specific product. Whatever platform you build on, fit this approach in (for the specific buttons and commands, **check the official docs**).
...
Notice that **half of this passage is about "what not to do" and "where to stop."** This isn't being wordy; this is the brakes you're installing on it.
...
| Fine to let it do itself | Must stop and wait for your nod |
| --- | --- |
...
2. **Did it stop where it should have?** Go back and confirm: those few "must-stop points," did it dutifully stop and wait for you at each. This one matters more than how nice the result looks.
...
| **Giving only a goal, no brakes** | Just saying "help me clean up my inbox" without "ask me before deleting"; it may delete all the way through. **The brakes go in with the goal, and written down before it acts** |
...
| **Forcing fixes inside a mess after it goes off track** | The more you fix, the more it skews. Just **roll back to a clean state and open a new conversation**, re-explaining "the pitfall just hit" as a premise (Chapters 13 and 30) |
...
- The most key thing in the prep stage isn't technical, it's **judgment**: pick a goal that's "multi-step but safe at every step," break the process down clearly, make a "must-stop list," and ready an environment you can roll back.
...
5. **[Advanced · Scenario]** Halfway through, the agent is clearly off track and getting messier. What's the best damage-control move?
   ...
   - B. Roll back to a clean state, open a new conversation, and re-explain "the pitfall just hit" as a premise
