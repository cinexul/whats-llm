...
| --- | --- | --- |
| **It starts contradicting itself** | This turn doesn't line up with earlier ones, front and back at odds | Contradictory old content piled on the desk leads it astray |
...
5) Long-term preferences you need every time -> hand them to the product's "memory /
   custom instructions" to remember (section 4, check the official docs)
```
...
| Setting "memory / preferences" means the model itself remembers me | It's the **product layer** stuffing preferences back into context each time, not the model itself remembering; check the official docs |
...
| Long-term preferences can only be repeated manually each time | Save them in the product's "memory / project / custom instructions" feature and it re-feeds them for you each time (check the official docs) |
...
- Long-term preferences: the model **itself has no memory across conversations**; to have it "remember," use the **product layer's** "memory / project / custom instructions", that's the product **stuffing preferences back into this context** each time, not the model itself remembering (check the official docs).
...
2. **[Basic · Concept]** Which is a typical signal that "it's time to open a new conversation"?
   ...
   > **Answer: B.** Contradicting itself, repeatedly forgetting requirements, getting more and more long-winded, are all signals that "the desk is full and noise drowns the signal", see them and switch to a clean desk. A, C, and D are exactly when it's in good shape or just starting, with no need to reopen.
...
3. **[Advanced · Misconception]** "When it starts drifting and forgetting my requirements, I just remind and correct it over and over in this conversation, and I can always pull it back." What's the problem?
   ...
   > **Answer: B.** The root of drifting is the desk being too full (Chapter 3), and adding more content to the original conversation only fills it more. The right move is to open a new conversation and lay out the still-needed information again. Whoever makes this mistake takes "context is full" for "it didn't understand, just say it again." C treats normal behavior as a fault; D is completely unrelated (higher temperature only makes it more unmoored, Chapter 8).
