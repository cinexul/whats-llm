〔Everything in this chapter about how Claude Code is installed, its commands, its interface, and what it can do will change as new versions ship, so always **check the official docs**. This chapter only covers "what it is, when to use it, how to judge it, and what to watch out for"; for word-for-word commands and deeper features, see **Appendix C** and the official docs.〕
...
## 1. The minimal mindset for getting started: recognize that it "has several faces"
...
Claude Code comes from **Anthropic**, and its core is **Claude**. Those two points are stable facts you can lock in. But it **takes more than one form**: you might run into it in a terminal, in a code editor plugin, in a desktop or web interface, or even as a cloud task; **some of it runs on your own machine, some of it runs in the cloud, the exact forms differ and are still changing, so check the official docs / the product's official forms.**

So the right mindset for getting started comes down to three lines:

- **Don't agonize over "what kind of software it actually is."** Go to the official docs to see "which form is recommended right now and how to install it," follow it once, and that's enough (**check the official docs**).
...
This is the one Claude Code design most worth understanding for an ordinary person, and it's the heart of this chapter.
...
**CLAUDE.md is the "onboarding notes" you write for this new coworker.** It's usually a text file kept in your project (**check the official docs for the exact filename, where it goes, and how it takes effect**), and inside it you write out this project's "rules" in plain words, for example:
...
Why insist on poking through this paper screen? Because once you get this, you can **predict its quirks** instead of putting blind faith in it:

- **A rule that isn't in the notes, it doesn't know.** That unwritten convention you assume "it should get": as long as it isn't in CLAUDE.md (or in this conversation), it still doesn't know it. Don't blame it for "forgetting," you didn't "feed" it.
...
| **Have it produce a plan before acting** | Many forms have a built-in mode of "give a plan first, you approve, then it changes" | This is "gate 1" from Chapter 24; a few dozen seconds blocks a few dozen minutes of rework (see Chapter 27) |
| **Permission confirmation** | Whenever it's about to do something "with consequences" (change a file, run a command) it usually stops and asks you | Don't mindlessly tap "approve": this is the safety valve Chapter 26 covers in detail |
...
You may hear these terms someday. Just get familiar with the faces for now; knowing "they're for extending Claude Code" is enough:
...
| --- | --- |
| Claude Code is some one fixed web page / software | It has many forms (terminal / IDE / desktop / web / cloud) and is still changing; **check the official docs** |
...
| Permission confirmation is just a hassle, tapping "approve" all the way is fine | That's like ripping out the brake; it's a safety valve protecting you (Chapters 26 and 30) |
...
- Claude Code comes from Anthropic and its core is Claude (stable facts); but it **takes many forms and is still changing**: for how to install and operate it, **check the official docs**.
...
- Common operations (investigate, produce a plan, permission confirmation, look at the diff, @ reference, halt anytime) all map to the workflow from Chapter 24; **check the official docs for how exactly to trigger them**.
...
1. **[Basic · Concept]** About Claude Code, which line is a **stable fact you can lock in**?
   ...
   > **Answer: B.** "The maker is Anthropic, the core is Claude" is a stable fact; while "what form it takes, how to install it, what commands to type" is the volatile part, so **check the official docs**. A and C pin down the volatile form; D gets the bar backwards: getting started only needs the workflow from Chapter 24.
...
3. **[Advanced · Misconception]** "I wrote CLAUDE.md, so the model now **remembers** my project." Where is this line inaccurate?
   ...
   > **Answer: B.** This is exactly the "**model itself vs. product layer**" that Chapters 3 and 13 stress over and over. CLAUDE.md is product-layer memory: it **lays the rules back out on the "desk" each time**, not the model growing a memory. Once you get this you can predict it: what isn't in the notes and wasn't said in this conversation, it still doesn't know.
...
6. **[Basic · Hands-on / Observation]** You don't have to actually install the tool: try writing a 5-to-8-line set of "onboarding notes" for a "project / matter" you know well (even "managing the household budget"). Write two or three each of the rules, minefields, and habits. Then ask yourself: which lines are "unwritten conventions" I used to assume "the other side should get," but that nobody actually knows unless you write them down?
   > **What you should notice:** This is exactly the essence of CLAUDE.md: **making the "taken-for-granted unwritten conventions" explicit**. You'll also feel how hard "being tight" is: write too much and nobody can get through it, write too little and you miss the key parts. As for what these notes are called inside Claude Code, where they go, and how they take effect, **check the official docs** (go deeper in Appendix C).
