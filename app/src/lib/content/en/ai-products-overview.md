...
What you're actually using:        the whole product experience
                ├─ is the interface smooth
                ├─ can it upload files / go online / sync
                ├─ does it remember preferences, make images, do voice...
                ├─ does the price and quota fit
...
The table below lists only the **relatively stable, unlikely-to-change "ownership"**: who makes it, what the core is. As for "can it do a particular thing right now," that kind of **specific capability updates extremely fast, so always check the official docs**. This book will not pin it down (pinning it down would soon be wrong anyway).

| Product | Maker | Core model | What you can safely remember |
| --- | --- | --- | --- |
| ChatGPT | OpenAI | GPT family | It's a product; the core is called GPT |
| Claude | Anthropic | Claude family | Claude is both a model family name and, often, the name used for its product |
| Gemini | Google | Gemini family | Google's product, often tied to Google's own services (search, office suite, etc.) |
| Others (various assistants, image/video tools, etc.) | Various | Varies | Many wrap some model and plug it into a particular use case |

> **Key point:** The only thing in this table worth remembering is the **"who makes it, what the core is" ownership**. That's a stable fact. **Never memorize "product X can do A, can't do B" as if it were settled.** What it can't do today, it may do next month; today's "exclusive feature" gets matched by others before long. The capability list is the **volatile part**, so always check the official docs.

A special reminder here: those online "feature comparison charts" and "who supports what," **glance at them for a rough idea, don't treat them as gospel.** They usually capture a snapshot at one point in time, and these products update every so often. **To actually confirm whether a feature exists and how to use it, the only reliable way is to check the official docs or official announcements.** This is the core action this chapter keeps trying to teach you.

## 3. How to choose by "task": ask yourself three questions

Instead of agonizing over "which is strongest," turn it around and start from **what you want to do**. The three questions below will mostly narrow your choice to the right direction.
...
This goes back to the chapter 17 test:

- **Personal, occasional, conversational** -> just use an off-the-shelf chat **product**. That's what this chapter is about.
...
Forget everything else in this chapter if you must, but **remember this section**, because it's the only weapon against going out of date.

Since this book (and any single review) can't possibly keep up with how fast products update, **what you really need is not "memorizing a product's feature list," but "learning to check current abilities yourself."** The method is plain:

```text
To know "whether a product can do a particular thing right now," a reliable order:
  1. Go straight to the official docs / help center / changelog  <- most reliable
  2. Try it yourself in the product (note: one try isn't the whole story; see next section)
  3. Read others' reviews -> treat only as a lead, still go back to 1 to confirm
```

- **Why do the official docs come first?** Because the things most likely to change, features, limits, prices, versions, only count once the maker says so. Any third-party information can lag or be inaccurate.
- **Why are reviews only a lead?** Because a review is usually the result for one person, at one time, with one way of using it. By the time you read it, things may have changed, and the reviewer's needs may not match yours.

> **Key point:** Build a habit: **anytime it touches "what it can do right now, how much it costs, what the limits are," your first instinct is "go check the official source," not "I think I remember..."** This action keeps outdated information from ever burning you. It's also exactly why this book keeps marking "**check the official docs**." Not out of laziness, but because these details belong to the maker and will change.

## 5. Don't judge a product by a "single experience"

One last mistake, especially easy to make, gets its own section.

Many people, after **one unpleasant experience**, sentence a product to death: "this AI is so dumb, I'm never using it again." That judgment is often unfair, for several reasons:

- **Updates are fast:** what you used last time may be a version from months ago, long since changed. Mapping an old impression onto the present is frequently wrong.
- **The same product runs hot and cold:** a model's answers carry randomness (temperature, chapter 8). The same question can be answered badly this time and fine after a reword or a retry. That doesn't mean "it just can't do it."
- **Is it a "usage" problem?** A lot of "it's so dumb" is really an unclear prompt (chapter 11), or going past the edge of its abilities (asking about things after its knowledge cutoff, chapter 9). Ask differently and the result can be worlds apart.
- **Wrong fit for the task:** taking a product to do something it's not good at, then sentencing it, isn't fair either.

> **Key point:** One experience is **just one data point, not a conclusion.** To judge whether a product really suits you, rely on the overall feel **after trying it several times on your real tasks, the right way**, not on one unlucky roll. A steadier approach: when you have an important task at hand, **run the same problem through two or three products** and compare them yourself. That beats any ranking.

## 6. So should you ignore "model strength" entirely?
...
Next chapter, we turn the lens to the outermost, most interesting layer of the chapter 17 map: the **Agent**, the AI that actually "takes action."
...
2. **[Basic · Concept]** About the mainstream AI products, which one is **relatively stable and worth remembering**?
   - A. Which specific features a product supports and which it doesn't
   - B. Each product's "ownership," e.g. ChatGPT is OpenAI's product, core is GPT
   - C. A product's price and quota this month
   - D. Which model is number one on the ranking right now
   > **Answer: B.** "Who makes it, what the core is" is a stable fact, worth remembering. A, C, D are all **volatile**: features, prices, limits, ranking positions all update extremely fast, so always check the official docs. Memorizing the volatile stuff as settled is exactly the mistake that goes out of date fastest.

3. **[Basic · Misconception]** "I used some AI last time and the answer was really dumb, so this product is just no good." Where's the mistake?
   - A. No mistake, one try is enough to conclude
   - B. One experience is just one data point: versions update, answers carry randomness, and it may be a bad prompt or a wrong fit for the task
   - C. It's dumb because the model has too few parameters
   - D. Switching products will definitely fix it
   > **Answer: B.** Products update fast (the old impression may be long out of date), answers carry randomness (temperature, chapter 8), and a lot of "it's so dumb" is really a usage problem (chapter 11) or asking about something beyond its abilities (after the knowledge cutoff, chapter 9). A is the classic "judge by one experience"; C and D pin the cause on a single factor without thinking.

4. **[Intermediate · Hands-on]** You want to confirm "whether an AI product right now supports a particular feature." What's the most reliable first step?
   - A. Check its **official docs / help center / changelog**
   - B. Find a review from six months ago and trust it
   - C. Ask "can it do X" on a forum and see what people say
   - D. Judge from your own impression from months ago
   > **Answer: A.** Features, limits, prices, versions are the things most likely to change, and **only count once the maker says so.** B and D can lag badly; C's crowd answers may not be accurate and may not match your needs. Trying it yourself in the product can be a backup, but still go back to the official source to confirm. This is exactly why this book keeps marking "check the official docs."

5. **[Basic · Scenario]** You can't live without a particular office suite, and now you want an AI to help with documents inside it. Besides "is the model strong," what should you especially prioritize?
   - A. Which AI is most famous online
