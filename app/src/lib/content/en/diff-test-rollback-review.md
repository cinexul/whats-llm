〔Wherever this chapter touches a specific product's commands, interface, or buttons, those change with each version, so always **check the official docs**. This chapter only covers the **shared, stable** guarding moves, and that part won't go out of date.〕
...
It looks like this (this is a **general illustration**; the exact colors and symbols differ a bit between tools, so **check the official docs**):
...
This is so common in programming that there's a phrase for it: "push down here, it pops up there." You have it fix A, it fixes A but accidentally bumps B. **And the place B breaks might not be in the diff you're looking at at all**, so reading the diff alone won't catch it.
...
1. **"What did it change?"** Looking at the diff, can you say "which spots it touched and roughly what for." If you can't, have it explain, or it **changed too much** (break it small, see Chapter 27).
...
> **Key point:** **"Don't merge what you can't follow" is the bottom line of this chapter, and of all of Part Four.** Finished isn't correct (Chapter 23); the diff lets you see what it changed, tests help you confirm it broke nothing, rollback gives you the confidence to "undo," and the three sign-off questions twist these three into one safety rope you can pull at hand. **Remember: the gatekeeping is yours, not its.**
...
| If it breaks things I'm done for | In an environment you can undo there's nothing to fear: locally with Git (cut a new branch first), web/cloud with the platform's built-in undo (**check the official docs**) |
...
- **Rollback = one move to undo**: let it act in an environment you can undo. **Locally with Git (cut a new branch first)**, web/cloud with **the platform's built-in undo (check the official docs).**
...
In the next chapter (Chapter 26) we'll talk through "acting in an environment you can undo" thoroughly: how the permission mechanism protects you, and why dangerous operations are always left for a human to nod through.
